...
export async function getUserTeam(
  ...
  try {
    ...
    if (error) {
      console.warn('Users_teams table not found - using fallback team configuration:', error.message)
      // Return fallback team configuration for development
      return {
        id: 'dev-team-id',
        name: 'Development Team',
        email: session?.user.email || 'dev@aigobuild.com',
        tier: 'pro'
      }
    }
    ...
    console.warn('Team lookup failed - using development fallback team')
    // Graceful fallback for missing database schema
    return {
      id: 'dev-team-fallback',
      name: 'AiGo Build Dev Team',
      email: session?.user.email || 'dev@aigobuild.com',
      tier: 'enterprise'
    }
  }
